...
class EventFrameBuffer:
    ...
    def _analyze_worker(self, event_type, prompt, frames, context):
        ...
        try:
            ...
            print(f"[EVENT VLM] Analisi {event_type}: {n_frames} frame → {self.model}")
            ...
            if description:
                ...
                print(f"[EVENT VLM] {event_type}: {description.strip()[:120]}...")
        except Exception as e:
    ...
    def on_walking_episode(self, episode):
        ...
        if duration < 2.0:
            return  # episodi troppo corti non valgono l'analisi
